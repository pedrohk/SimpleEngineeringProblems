...
object DPK15_impl_06 {

  class Friends {
    private val buffer = ArrayBuffer[Person]()

    def add(friend: Person): Unit = {
      if (!buffer.exists(_.getName() == friend.getName())) {
        buffer += friend
      }
    }

    def remove(friend: Person): Unit = {
      val idx = buffer.indexWhere(_.getName() == friend.getName())
      if (idx >= 0) buffer.remove(idx)
    }
